...
// TODO:
//  - Proof of possession.
//  - Consider including POP in Coze.
...
// Dig is a digest that includes that algorithm used to derive the digest value.
type Dig struct {
	alg coze.HshAlg
	dig coze.B64
}

// Tran, a Transaction includes all signatures required to result in a new block. A
...
// Key for each coze is `czd`
type Tran map[B64s][]coze.Coze

// A derivation is the identifier for auth using a particular hashing algorithm.
// The abstract block ID, may have multiple derivations, one for each algorithm.
type Derivation struct {
	Dig
	bid coze.B64 // A particular digest Block ID using the hash alg of the derivation.
}

// Derivations is a map of derivations for a particular block.
type Derivations map[B64s]Derivation

// Account represents a Cyphrpass user account (Levels 1–3).
//
//   - AR is the permanent Account Root.
//
//   - BID is the current block id, the Merkle Root of all auth and data.
//
//   - AS, Auth State, is the current Merkle Root of auth for current block: all active key
//     thumbprints, all transactions, rules, (and potentially a nonce).
//     An AS changes as auth changes, and is below the master MR for a block, BID.
//
//   - DS, Data State, is the current Merkle Root of auth for current block: all active key
//     thumbprints, all transactions, rules, (and potentially a nonce).
//     An AS changes as auth changes, and is below the master MR for a block, BID.
//
...
type Account struct {
	// InternalAccountID coze.B64 `json:"internal_account_id"`
	// InternalBlockID   coze.B64 `json:"internal_block_id"`

	AR     Derivations `json:"ar"`
	Latest B64s        // Latest block

	Keys        map[B64s]*coze.Key `json:"keys,omitempty"`
	RevokedKeys map[B64s]int64     `json:"revoked_keys,omitempty"`
	Tran        []*coze.Coze       `json:"transactions,omitempty"` // Transactions
}
...
type Block struct {
	BID Derivations `json:"bid"`
	AS  Derivations `json:"as"` // Auth State
	DS  Derivations `json:"ds"` // Data State
...
var ActiveKeys map[B64s]coze.Key  // Flat with all active keys
var RevokedKeys map[B64s]coze.Key // Flat with all revoked keys
...
func NewMultiAccount(initialKeys ...*coze.Key) (*Account, error) {
	...
	acc := &Account{
		Keys:        make(map[B64s]*coze.Key),
		RevokedKeys: make(map[B64s]int64),
		Tran:        make([]*coze.Coze, 0),
	}
	...
	for _, key := range initialKeys {
		...
		acc.Keys[B64s(key.Tmb.String())] = key
	}
