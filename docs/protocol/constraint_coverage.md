...
| `[transaction-pre-required]`    | Mutations must have `pre`             | 🟡 TESTABLE       | Can override pre to empty string                                           |
| `[data-action-no-pre]`          | Actions must NOT have `pre`           | 🔶 NEEDS_OVERRIDE | Need `inject_pre` override on action                                       |
...
| `[commit-one-or-more]`          | Commit must have ≥1 coz               | 🔶 NEEDS_OVERRIDE | Need empty-commit override; `EmptyCommit` error exists                     |
...
- ✅ TESTED: 12 (+2: `[revoke-self-signed]`, `[no-revoke-non-self]`)
- 🟡 TESTABLE: 1 (`[transaction-pre-required]`)
- 🔶 NEEDS_OVERRIDE: 2 (`[data-action-no-pre]`, `[commit-one-or-more]`)
...
| `[no-empty-mr]`                     | 🔶 NEEDS_OVERRIDE | Need empty-key genesis; `EmptyMultihash` error exists |
