...
| `vtx.clone()` in `principal.rs` L780                                         | LOW      | Borrow checker workaround during restructuring                                                                      | Verify if clone is needed; if not, revert to move. If needed, add comment explaining why             |   [x]    |
...
| Generator functions still named `generate_single_step`/`generate_multi_step` | LOW      | Cosmetic rename to `_commit` would add noise to type-migration diff                                                 | Rename in Phase 4b DRY cleanup                                                                       |   [x]    |
| `build_pay_json` unconditionally sets `commit=true`                          | LOW      | All current commits have 1 tx; correct for current invariant                                                        | Revisit if multi-tx-per-commit tests are added                                                       |   [x]    |
| Dead `pre` fallback in `build_pay_json` for `PrincipalCreate.id`             | LOW      | Defensive coding; `current_as` is always provided in practice                                                       | Remove fallback branch or convert to error — `pre` is CS, not AS, so fallback is semantically wrong  |   [x]    |
...
| Unused `_alg` return from `build_action_pay_json`                            | LOW      | Helper returns `(Vec<u8>, String)` but alg unused at both call sites                                                | Remove second tuple element if no consumer materializes                                              |   [x]    |
