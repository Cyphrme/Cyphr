...
| BlobStore backend        | redb (v4.1)                             | Pure Rust, ACID, MVCC, single-file, stable since v1.0 (2023). Sled rejected (alpha stability). RocksDB rejected (C++ dependency).                                                                                                                                                                              |
...
| IV. Backing services   | redb and SQLite are attached resources — paths configured, not compiled in. Swappable via trait.                                                                                                  |
| V. Build, release, run | `cargo build --release` → binary artifact → run with config. Strict separation.                                                                                                                   |
| VI. Processes          | Stateless process. All durable state in storage (redb + SQLite). In-process principal cache is ephemeral and reconstructible.                                                                     |
...
| Dual-backend crash inconsistency (redb ↔ SQLite) | MEDIUM   | Mitigated    | BlobStore IS the durable log; index is always rebuildable. Recovery path (scan BlobStore for unindexed blobs) implemented in Phase 2.  |
...
| redb is production-ready                          | —        | ✅ Validated | Stable since v1.0 (June 2023), currently v4.1 (April 2026), ACID, MVCC, pure Rust.                                                     |
...
1. **Phase 1: BlobStore Foundation** — content-addressed blob storage with BLAKE3
   - [ ] `BlobStore` trait definition (`cyphr-storage/src/blob/mod.rs`)
     ...
     - [ ] `iter() -> Result<impl Iterator<Item = (Blake3Hash, Vec<u8>)>>`
   - [ ] `RedbBlobStore` implementation
     - [ ] Single redb table: `blake3_hash -> raw_bytes`
     - [ ] File-based configuration (storage directory path)
