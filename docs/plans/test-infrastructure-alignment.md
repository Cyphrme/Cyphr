...
| :------------------------------------------------------------------------------ | :------- | :------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------- | :------: |
| `injectKey()` re-serializes tx JSON via `map[string]json.RawMessage` round-trip | HIGH     | Bridge pattern for Go storage layer which expects per-entry embedded key                     | Eliminated: runner now uses `entriesFromCommits` with inline `embedKey`            |    ✓     |
