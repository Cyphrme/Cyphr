...
| Go `ErrUnsupportedAlgorithm` reused for malt proof errors — same semantic imprecision as the Rust variant above. Malt's own errors (index OOB, empty tree) propagate directly, but the unknown-algorithm case returns the wrong sentinel.             | LOW      | Same rationale as Rust: pre-alpha sentinel set; Go has no `fmt.Errorf`-wrapping variant for malt errors yet.                                              | Introduce a `ErrProof` sentinel or use `fmt.Errorf("cyphr: proof: %w", err)` wrapping when Go error taxonomy is revisited alongside Rust.                                        |    ☐     |
| Go accessor named `CommitTreesAccessor()` instead of `CommitTrees()` to avoid shadowing the `CommitTrees` type alias. Unconventional Go naming — the method reads awkwardly.                                                                          | LOW      | Go's type system doesn't allow a method and a type alias to share the same name in the same package. Renaming the type would be a larger breaking change. | Consider renaming the type to `CommitTreeMap` or similar during a broader naming pass, enabling the method to use the natural name `CommitTrees()`.                              |    ☐     |
